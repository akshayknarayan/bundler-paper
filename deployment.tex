...
%
Dhamdhere \etal found that~\cite{inferring-interdomain-congestion} inter-domain bottlenecks (such as the red bottleneck link in Figure~\ref{fig:deploy:arch}) do exist\fc{<-weak}, and
...
\paragrapha{Litmus test for \name's applicability}
Another way of understanding when \name is useful, which incorporates the three conditions above, is the following litmust test:
