...
%The next question that arises is, does such in-network congestion occur in practice? 
A recent measurement study~\cite{inferring-interdomain-congestion} indicates that inter-domain bottlenecks (such as the red bottleneck link in Figure~\ref{fig:deploy:arch}) do exist.
% \fc{seems like a weak statement, esp since we're starting with a question. can we say something stronger than exist? or delete the question sentence}\an{removed sentence}. 
