...
\radhika{add a line or two on what the control plane does.}
...
To highlight the simplicity of our approach\footnote{and because we are lazy programmers}, \radhika{cut the part before, start from here: ``For our prototype,''} we implement the inbox data-plane using Linux \texttt{tc}~\cite{tc}, and we limit our implementations of scheduling policy to those already widely available in the Linux kernel.
...
condition \radhika{fix}. (3) It sends a netlink message to the \inbox process running in user-space, which records the packet
...
(7) Asynchronously, the \inbox invokes the congestion control algorithm every $10$ms \radhika{seems arbitrary -- should it be RTT?} via \texttt{libccp},
...
and sets the rate of the qdisc to the minimum of the explicit rate and this effective rate \radhika{are you using this for any scheme?}.
