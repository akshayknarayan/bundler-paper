...
\name consists of sender-side (\inbox) and receiver-side (\outbox) functionality, as in Figure~\ref{fig:deploy:arch}. As detailed in \S\ref{sec:desing}, the \inbox of one domain establishes a pairing with the \outbox of another domain when sending traffic to it. We define a \emph{bundle} to be a group of flows that share the same \inbox-\outbox pair.
