...
\name consists of sender-side (\inbox) and receiver-side (\outbox) functionality, as in Figure~\ref{fig:deploy:arch}.
We define a \emph{bundle} to be a group of flows that share the same \inbox in their sending domain and the same \outbox in their receiving domain (what we call a \inbox-\outbox pair).
