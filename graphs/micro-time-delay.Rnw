\begin{figure}
    \centering
<<micro:time-delay,cache=TRUE,echo=FALSE,warning=TRUE,verbose=TRUE,fig.width='\\columnwidth',fig.height=6>>=
...
data <- read.csv("minrtt-50-delay-dist.data", sep=" ", header=FALSE, colClasses=c("numeric","numeric","numeric","numeric", "numeric", "numeric", "numeric"))
...
p1 <- ggplot() + 
    ...
    scale_color_manual(name="",labels=c(measured="Measured", true="Bottleneck"), values=c(true="#2c7bb6", measured="#d7191c")) +
...
data <- read.csv("minrtt-delay-dist.data", sep=" ", header=FALSE, colClasses=c("numeric","numeric","numeric","numeric","numeric","numeric","numeric"))
colnames(data) <- c("t", "mm_delay", "inbox_delay", "diff_delay", "mm_thru", "inbox_thru", "diff_thru")
