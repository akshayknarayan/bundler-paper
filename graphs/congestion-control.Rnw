\begin{figure}
    \centering
<<eval:cc,fig.showtext=TRUE,cache=TRUE,echo=FALSE,message=FALSE,warning=FALSE,fig.width='\\columnwidth',fig.height=3>>=
library(ggplot2)
library(showtext)
...
ccCopaPerf = quantile(df[df$Alg=="copa_sfq",]$Slowdown, c(0.5, 0.99))
ccNimbusPerf = quantile(df[df$Alg=="nimbus_sfq",]$Slowdown, c(0.5, 0.99))
ccBBRPerf = quantile(df[df$Alg=="bbr_sfq",]$Slowdown, c(0.5, 0.99))
ccBaselinePerf = quantile(df[df$Alg=="nobundle_fifo",]$Slowdown, c(0.5, 0.99))
@
    ...
    \label{fig:eval:cc}
\end{figure}
\newcommand{\ccCopaMedian}{\Sexpr{ccCopaPerf[0]}}
\newcommand{\ccNimbusMedian}{\Sexpr{ccNimbusPerf[0]}}
\newcommand{\ccBBRMedian}{\Sexpr{ccBBRPerf[0]}}
\newcommand{\ccBaselineMedian}{\Sexpr{ccBaselinePerf[0]}}
