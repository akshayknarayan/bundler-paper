\begin{figure}
    \centering
    <<micro:time,cache=TRUE,echo=FALSE,warning=TRUE,verbose=TRUE,fig.width='\\columnwidth',fig.height=4>>=
...
p1 <- ggplot() + 
    ...
    scale_color_manual(name="",labels=c(raw="Measured", windowed="Measured (Windowed)", true="Bottleneck"), values=c(raw="#abd9e9", windowed="#2c7bb6", true="#d7191c")) +
    scale_x_continuous(breaks=seq(0,max(thru$V1),by=1)) +
    scale_y_continuous(breaks=seq(0,140,by=12)) +
    xlab("Time (seconds)") + 
    ylab("Rate (Mbps)") + 
    coord_cartesian(xlim=c(20,25), ylim=c(0,140)) + 
