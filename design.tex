...
In this section, we first describe the key idea behind \name's design, followed by discussing our specific design choices. 
...
Guided by our key requirement of \emph{deployment and management ease}, our design choices resulted in the following nice properties: 
...
\vspace{5pt}
\noindent We discuss our design choices in more details below. 
...
\paragraphi{(2) Infrequent measurements} Sending an out-of-band feedback message for every packet received by the \outbox would result in high communication overhead. Furthermore, conducting measurements on every outgoing packet at the \inbox would require maintaining state for each of them, which can be expensive, especially at high bandwidth-delay products. We, therefore, conduct measurements on a few sampled packet, at least once per RTT, which is sufficient for most congestion control algorithms~\cite{ccp}. 

\paragraphi{(3) Independent coordination} The \inbox and \outbox need to agree upon the set of sampled packets over which measurements will be conducted (to ensure overlap between the packets for which the \outbox generates feedback and the packets for which the \inbox maintains state). It is lucrative to simply mark such packets at the \inbox. However, past studies~\cite{ipoptions, mboxbadness, quic} have shown how such strategies may fail, as many middleboxes have a tendency of dropping or re-writing packets with unfamiliar header fields. 
...
We, therefore, devise a technique to sample packets solely based on the hash computed over certain fields in the packet header, which allows the \inbox and the \outbox to \emph{independently} sample the same set of packets.
