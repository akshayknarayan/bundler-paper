...
Our design choices were guided by our key requirement of \emph{deployment and management ease}. They provide the following nice properties:
...
%Second, there can be significant differences in the RTT values between flows that are part of the same bundle, but belong to different receiving endhosts, which would make these values an unreliable congestion signal for the aggregated bundle. 
 Instead, \name's two-sided design allows measurement of congestion signals to be conducted between solely an \pair. We highlight the key features of our two-sided measurement technique below, and describe it in more detail in \S\ref{s:measurement}. \fc{train of thought is not as clear to me here}
