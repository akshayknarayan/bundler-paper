...
A trivial strategy for inducing queuing at the \name, for effective scheduling of traffic within a bundle, is to throttle the outgoing rate of the bundle to a small value. However, this would adversely affect the performance of the bundled traffic, by reducing its share of bandwidth in the network, and potentially leaving the bottleneck links under-utilized. Instead, the rate needs to be set such that bottleneck links remain fully utilized and the bundled traffic competes fairly with the cross traffic from other domains. 
%In order to prevent under-utilization of the bottleneck links in the network, \name needs to occupy . 
We make a simple, but powerful, observation: congestion control algorithms perform exactly this calculation! 
...
Figure~\ref{fig:design:shift-bottleneck} illustrates this method for a single flow traversing a bottleneck link in the network: in Figure~\ref{fig:design:shift-bottleneck:before}, packets from the end-hosts are queued in the network, while the queue at the edge is unoccupied. In contrast, the \name implementation in Figure~\ref{fig:design:shift-bottleneck:after} (described in \S\ref{s:impl}) maintains low queuing delays at the bottleneck queue by estimating and sending at the bottleneck rate. 
