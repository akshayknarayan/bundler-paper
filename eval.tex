...
This scenario highlights the benefits of using \name when the congestion on the bottleneck link in the network is self-inflicted by a single customer. We explore the effects of queues at the bottleneck caused by other cross-traffic in \S\ref{s:robust}.
...
%
\an{check this text}
For the short requests, terminating TCP connections does not yield benefits: with or without termination, they finish in a few RTTs and never leave slow start.
