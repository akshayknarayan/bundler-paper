...
\Para{Enabling packet scheduling in wide-area networks} PIFO~\cite{pifo} is programmable scheduler that can be configured differently at the routers to express different policies. Not only does it require a new router hardware, but it also suffers from the issue of network operators having limited visibility into customers' traffic to choose appropriate policies, and limited incentives to enforce them. UPS~\cite{ups} goes a step further by allowing different scheduling policies to be expressed from the edge via header initialization. 
