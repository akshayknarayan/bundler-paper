...
The \outbox, on the other hand, tracks only the total number of bytes received since the last epoch boundary. Upon the arrival and identification of the same epoch boundary packet, $p_i$, it immediately sends a feedback message to the \inbox containing similar information: (i) its hash, $h(p_i)$, (ii) the time when it was received (same as the current time), $t_{recv}(p_i)$, and (iii) the number of bytes received since the last epoch boundary, $b_{recv}(p_i)$. 
...
Upon receiving the feedback message for the epoch boundary, $p_i$, the \inbox uses its recorded state to compute the RTT and the rates at which packets are sent and received as below, and then clears the recorded state of all boundary packets preceding $p_i$.
