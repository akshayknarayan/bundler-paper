...
\label{s:measure:microbench}
    ...
    In Figure~\ref{fig:micro:time}, we look closely at a five second segment from one from one experiment in our evaluation (\S\ref{s:eval}) and compare our estimate of the receive rate and RTT to the actual values at the bottleneck router.
    It is easy to see that during this short time segment \name is able to track the values closely over time. 
    ...
    In Figure~\ref{fig:micro:thru}, for both t we compute the difference between our estimate of the receive rate 
