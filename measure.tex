...
\fc{maybe explain why these signals}
...
The combination of IP ID, destination port and destination IP (used by our prototype implementation in \ref{s:impl}) is one possible option that satisfies all of the above requirements.

\begin{outline}
\1 footnote: we expect that the precise set of fields used will depend on specific deployment considerations.
\1 We discuss the possible existence of on-path tunnelling separately below.
\1 We propose the use of IP ID, destination port, and destination IP as the minimal viable set of fields.
    \2 destination port and ip are required for correct routing behavior. They cannot be changed inside the network without compromising correctness
    \2 Previous proposals~\cite{ip-traceback} have relied of IP ID staying constant through the path
\1 A note on Internet architecture
    \2 We know our proposal is a bit hacky. There are a number of potential pitfalls to this approach.
        \3 mid-path tunnelling, NAT, etc
            \4 IPv6 has no IP ID field
        \3 architectural in nature
    \2 An architectural solution to this problem could use IP tunnels (\eg IP-in-IP) to ensure that packets arrive at the \outbox without modification after leaving the \inbox.
        \3 And provide mechanism for their unique identification
        \3 This approach is consistent with a recent proposal~\cite{trotsky} to enable such properties from the Internet architecture
\end{outline}
