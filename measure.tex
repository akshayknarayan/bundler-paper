...
However, packet modification is unnecessary; since the same packets pass through \inbox and \outbox, uniquely identifying a given pattern of packets is sufficient to meet our requirements. In this scheme, we hash a subset of the packet header and check if the hash is a multiple of the desired sampling period. Thus, the packet header subset must have the following properties:
