...
\subsection{Fairness Control}\label{s:qctl:fair}
...
If we simply \emph{let the traffic pass}, these end-host congestion controllers will naturally compete fairly with cross traffic, just as traffic in the Internet does today. 
...
\subsection{Queue Controller}\label{s:qctl:ctl}
To support active probing while also letting the traffic pass, the \inbox should hold back some packets while letting the remainder pass.
